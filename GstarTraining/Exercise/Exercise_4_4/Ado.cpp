...
long vartol(const _variant_t& var)
{
	...
	case VT_LPSTR://字符串
	case VT_LPWSTR://字符串
		value = _ttol((LPCTSTR)(_bstr_t)var);
		break;
	case VT_NULL:
	case VT_EMPTY:
		value = 0;
...
/*========================================================================
	...
	Remarks:	请参考 CAdoRecordSet 类的Open方法. 返回的 Recordset 对象始
			终为只读、仅向前的游标.
==========================================================================*/
_RecordsetPtr CAdoConnection::Execute(LPCTSTR lpszSQL, long *lRecordsAffected, long lOptions)
{
	ASSERT(m_pConnection != NULL);
	...
	try
	{
        recordSet = m_pConnection->Execute(_bstr_t(lpszSQL), &vt, lOptions);
        *lRecordsAffected = vt.lVal;
		return recordSet;
	}
	catch (_com_error e)
	{
